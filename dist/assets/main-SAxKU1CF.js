(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))i(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const a of r.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&i(a)}).observe(document,{childList:!0,subtree:!0});function t(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(o){if(o.ep)return;o.ep=!0;const r=t(o);fetch(o.href,r)}})();class v{constructor(){this.routes=[],this.currentUrl="",window.addEventListener("hashchange",()=>{console.log("Hash changed to:",window.location.hash),this._loadRouteWithTransition()})}addRoute(e,t,i={}){return this.routes.push({url:e,callback:t,requiresAuth:i.requiresAuth||!1,guestOnly:i.guestOnly||!1}),this}setFallback(e){return this.fallbackCallback=e,this}_loadRouteWithTransition(){document.startViewTransition?document.startViewTransition(()=>{this._loadRoute()}):(this._applyFallbackTransition(),this._loadRoute())}_applyFallbackTransition(){const e=document.getElementById("mainContent");e&&(e.style.opacity="0",e.style.transition="opacity 0.3s ease",setTimeout(()=>{e.style.opacity="1"},300))}_loadRoute(){const t=window.location.hash.slice(1)||"/";this.currentUrl=t;const i=localStorage.getItem("token")!==null;console.log("Loading route:",t,"Auth:",i);const o=this.routes.find(r=>r.url===t);if(o){if(o.requiresAuth&&!i&&t!=="/masuk"){console.log("Halaman memerlukan login. Mengalihkan ke halaman login..."),this.navigateTo("/masuk");return}if(o.guestOnly&&i&&t!=="/"){console.log("Halaman hanya untuk tamu. Mengalihkan ke beranda..."),this.navigateTo("/");return}try{o.callback()}catch(r){console.error("Error in route callback:",r)}}else this.fallbackCallback&&(console.log("Route tidak ditemukan, menggunakan fallback"),this.fallbackCallback())}navigateTo(e){if(console.log("Navigating to:",e),(window.location.hash.slice(1)||"/")===e){console.log("Already on this URL, skipping navigation");return}window.location.hash=e}init(){console.log("Router initialized"),this._loadRouteWithTransition(),this._addViewTransitionStyles()}_addViewTransitionStyles(){if(!document.getElementById("viewTransitionStyles")){const e=document.createElement("style");e.id="viewTransitionStyles",e.textContent=`
      ...
      `,document.head.appendChild(e)}}}window.Router=v;const b=new v;window.router=b;document.addEventListener("DOMContentLoaded",()=>{console.log("Setting up navigation link handlers"),document.body.addEventListener("click",n=>{const e=n.target.closest("a");if(!e)return;const t=e.getAttribute("href");if(t&&t.startsWith("#/")){n.preventDefault(),console.log("Navigation link clicked:",t);const i=t.substring(1);b.navigateTo(i)}})});class A{constructor(){this.stream=null,this.capturedImageDataUrl=null}async startCamera(e){try{return console.log("Starting camera..."),this.stopCamera(),this.stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:1280},height:{ideal:720},facingMode:"environment"}}),e.srcObject=this.stream,console.log("Camera started successfully"),!0}catch(t){return console.error("Error starting camera:",t),!1}}captureImage(e,t){try{return console.log("Capturing image..."),t.width=e.videoWidth,t.height=e.videoHeight,t.getContext("2d").drawImage(e,0,0,t.width,t.height),this.capturedImageDataUrl=t.toDataURL("image/jpeg",.8),this.stopCamera(),console.log("Image captured successfully"),this.capturedImageDataUrl}catch(i){return console.error("Error capturing image:",i),null}}stopCamera(){this.stream&&(console.log("Stopping camera..."),this.stream.getTracks().forEach(e=>{e.stop()}),this.stream=null,console.log("Camera stopped"))}async getCapturedImageBlob(){if(!this.capturedImageDataUrl)return console.error("No captured image available"),null;try{const t=await(await fetch(this.capturedImageDataUrl)).blob();return console.log("Image blob created:",t.size,"bytes"),t}catch(e){return console.error("Error creating image blob:",e),null}}async getImageBlobFromCanvas(e){return new Promise((t,i)=>{e.toBlob(o=>{o?(console.log("Canvas blob created:",o.size,"bytes"),t(o)):i(new Error("Failed to create blob from canvas"))},"image/jpeg",.8)})}}const P=new A;window.cameraHelper=P;class C{constructor(){this.dbName="PetaBicaraDB",this.dbVersion=1,this.db=null,this.stores={favorites:"favorites",offlineStories:"offlineStories",settings:"settings"}}async init(){return new Promise((e,t)=>{console.log("Initializing IndexedDB...");const i=indexedDB.open(this.dbName,this.dbVersion);i.onerror=()=>{console.error("IndexedDB initialization failed:",i.error),t(i.error)},i.onsuccess=()=>{this.db=i.result,console.log("IndexedDB initialized successfully"),e(this.db)},i.onupgradeneeded=o=>{console.log("Upgrading IndexedDB schema...");const r=o.target.result;r.objectStoreNames.contains(this.stores.favorites)||(r.createObjectStore(this.stores.favorites,{keyPath:"id"}).createIndex("createdAt","createdAt",{unique:!1}),console.log("Favorites store created")),r.objectStoreNames.contains(this.stores.offlineStories)||(r.createObjectStore(this.stores.offlineStories,{keyPath:"id",autoIncrement:!0}).createIndex("timestamp","timestamp",{unique:!1}),console.log("Offline stories store created")),r.objectStoreNames.contains(this.stores.settings)||(r.createObjectStore(this.stores.settings,{keyPath:"key"}),console.log("Settings store created"))}})}async performTransaction(e,t,i){return this.db||await this.init(),new Promise((o,r)=>{const a=this.db.transaction([e],t),s=a.objectStore(e);a.oncomplete=()=>{console.log(`Transaction completed for ${e}`)},a.onerror=()=>{console.error(`Transaction failed for ${e}:`,a.error),r(a.error)};try{const l=i(s);l?(l.onsuccess=()=>o(l.result),l.onerror=()=>r(l.error)):o()}catch(l){r(l)}})}async addToFavorites(e){console.log("Adding story to favorites:",e.id);const t={id:e.id,name:e.name,description:e.description,photoUrl:e.photoUrl,createdAt:e.createdAt,lat:e.lat,lon:e.lon,addedToFavoritesAt:new Date().toISOString()};return this.performTransaction(this.stores.favorites,"readwrite",i=>i.add(t))}async removeFromFavorites(e){return console.log("Removing story from favorites:",e),this.performTransaction(this.stores.favorites,"readwrite",t=>t.delete(e))}async getAllFavorites(){return console.log("Getting all favorite stories"),this.performTransaction(this.stores.favorites,"readonly",e=>e.getAll())}async isFavorite(e){return this.performTransaction(this.stores.favorites,"readonly",t=>t.get(e)).then(t=>!!t)}async getFavoritesCount(){return this.performTransaction(this.stores.favorites,"readonly",e=>e.count())}async saveOfflineStory(e){console.log("Saving story for offline submission");const t={description:e.description,photoBlob:e.photoBlob,lat:e.lat,lon:e.lon,token:localStorage.getItem("token"),timestamp:new Date().toISOString(),synced:!1};return this.performTransaction(this.stores.offlineStories,"readwrite",i=>i.add(t))}async getUnsyncedStories(){return console.log("Getting unsynced offline stories"),this.performTransaction(this.stores.offlineStories,"readonly",e=>e.getAll()).then(e=>e.filter(t=>!t.synced))}async markStorySynced(e){return console.log("Marking story as synced:",e),this.performTransaction(this.stores.offlineStories,"readwrite",async t=>{const i=await new Promise((o,r)=>{const a=t.get(e);a.onsuccess=()=>o(a.result),a.onerror=()=>r(a.error)});if(i)return i.synced=!0,t.put(i)})}async removeOfflineStory(e){return console.log("Removing offline story:",e),this.performTransaction(this.stores.offlineStories,"readwrite",t=>t.delete(e))}async saveSetting(e,t){console.log("Saving setting:",e);const i={key:e,value:t,updatedAt:new Date().toISOString()};return this.performTransaction(this.stores.settings,"readwrite",o=>o.put(i))}async getSetting(e){return this.performTransaction(this.stores.settings,"readonly",t=>t.get(e)).then(t=>t?t.value:null)}async clearAllData(){console.log("Clearing all IndexedDB data");const t=Object.values(this.stores).map(i=>this.performTransaction(i,"readwrite",o=>o.clear()));return Promise.all(t)}async getDatabaseInfo(){this.db||await this.init();const e={name:this.dbName,version:this.dbVersion,stores:{}};for(const[t,i]of Object.entries(this.stores))try{const o=await this.performTransaction(i,"readonly",r=>r.count());e.stores[t]={name:i,count:o}}catch(o){console.error(`Error getting info for store ${i}:`,o),e.stores[t]={name:i,count:0,error:o.message}}return e}async exportFavorites(){const e=await this.getAllFavorites();return{exportDate:new Date().toISOString(),version:this.dbVersion,favorites:e}}async importFavorites(e){if(!e.favorites||!Array.isArray(e.favorites))throw new Error("Invalid favorites data format");console.log(`Importing ${e.favorites.length} favorites`);const t=e.favorites.map(i=>this.addToFavorites(i).catch(o=>{console.warn(`Failed to import favorite ${i.id}:`,o)}));return Promise.allSettled(t)}}const y=new C;document.addEventListener("DOMContentLoaded",()=>{y.init().catch(n=>{console.error("Failed to initialize IndexedDB:",n)})});window.indexedDBHelper=y;class B{constructor(){this.vapidPublicKey="BCCs2eonMI-6H2ctvFaWg-UYdDv387Vno_bzUzALpB442r2lCnsHmtrx8biyPi_E-1fSGABK_Qs_GlvPoJJqxbk",this.apiBaseUrl="https://story-api.dicoding.dev/v1",this.basePath="/Submission-Intermediate",this.isSupported="serviceWorker"in navigator&&"PushManager"in window,this.registration=null,this.subscription=null,console.log("Push notifications supported:",this.isSupported)}async init(){if(!this.isSupported)return console.warn("Push notifications are not supported"),!1;try{return this.registration=await navigator.serviceWorker.ready,console.log("Service worker ready for push notifications"),this.subscription=await this.registration.pushManager.getSubscription(),this.subscription&&(console.log("Existing push subscription found"),await this.sendSubscriptionToStoryAPI(this.subscription)),!0}catch(e){return console.error("Error initializing push notifications:",e),!1}}async requestPermission(){if(!this.isSupported)throw new Error("Push notifications are not supported");let e=Notification.permission;if(e==="default"&&(e=await Notification.requestPermission()),console.log("Notification permission:",e),e!=="granted")throw new Error("Notification permission denied");return e==="granted"}async subscribe(){try{if(!await this.requestPermission())throw new Error("Permission not granted");if(!this.registration&&!await this.init())throw new Error("Failed to initialize service worker");const t=this.urlBase64ToUint8Array(this.vapidPublicKey);return this.subscription=await this.registration.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:t}),console.log("Push subscription successful:",this.subscription),await this.sendSubscriptionToStoryAPI(this.subscription),await this.saveSubscriptionLocally(this.subscription),await this.showServiceWorkerNotification("Notifikasi Aktif!","Kamu akan mendapat notifikasi untuk cerita baru."),this.subscription}catch(e){throw console.error("Error subscribing to push notifications:",e),e}}async unsubscribe(){try{return this.subscription||this.registration&&(this.subscription=await this.registration.pushManager.getSubscription()),this.subscription&&(await this.removeSubscriptionFromStoryAPI(this.subscription),await this.subscription.unsubscribe())?(console.log("Push notification unsubscribed successfully"),await this.removeSubscriptionLocally(),this.subscription=null,!0):!1}catch(e){throw console.error("Error unsubscribing from push notifications:",e),e}}async sendSubscriptionToStoryAPI(e){try{console.log("Sending subscription to Story API...");const t=localStorage.getItem("token");if(!t)throw new Error("No authentication token found");const i={endpoint:e.endpoint,keys:{p256dh:this.arrayBufferToBase64(e.getKey("p256dh")),auth:this.arrayBufferToBase64(e.getKey("auth"))}};console.log("Subscription data to send:",i);const r=await(await fetch(`${this.apiBaseUrl}/notifications/subscribe`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify(i)})).json();if(console.log("Story API subscription response:",r),r.error)throw new Error(r.message||"Failed to subscribe to Story API");return r.data&&r.data.id&&localStorage.setItem("story-api-subscription-id",r.data.id),r}catch(t){throw console.error("Error sending subscription to Story API:",t),t}}async removeSubscriptionFromStoryAPI(e){try{console.log("Removing subscription from Story API...");const t=localStorage.getItem("token");if(!t){console.warn("No authentication token found for unsubscribe");return}const i={endpoint:e.endpoint},r=await(await fetch(`${this.apiBaseUrl}/notifications/subscribe`,{method:"DELETE",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify(i)})).json();console.log("Story API unsubscribe response:",r),r.error?console.warn("Failed to unsubscribe from Story API:",r.message):localStorage.removeItem("story-api-subscription-id")}catch(t){console.error("Error removing subscription from Story API:",t)}}async showServiceWorkerNotification(e,t,i={}){if(!this.registration)return console.warn("Service Worker not registered"),!1;try{const r={...{body:t,icon:`${this.basePath}/icons/icon-192x192.png`,badge:`${this.basePath}/icons/icon-72x72.png`,vibrate:[100,50,100],data:{dateOfArrival:Date.now(),primaryKey:Math.random(),url:`${this.basePath}/`},actions:[{action:"open",title:"Buka App",icon:`${this.basePath}/icons/icon-72x72.png`},{action:"close",title:"Tutup"}],requireInteraction:!1,silent:!1,tag:"peta-bicara-notification"},...i};return await this.registration.showNotification(e,r),console.log("Service Worker notification shown successfully"),!0}catch(o){return console.error("Error showing Service Worker notification:",o),!1}}showBasicNotification(e,t,i=null){if(!this.isSupported)return console.warn("Notifications not supported"),!1;if(Notification.permission!=="granted")return console.warn("Notification permission not granted"),!1;try{const o=i||`${this.basePath}/icons/icon-192x192.png`,r=new Notification(e,{body:t,icon:o,badge:`${this.basePath}/icons/icon-72x72.png`,vibrate:[100,50,100],data:{dateOfArrival:Date.now(),primaryKey:Math.random(),url:`${this.basePath}/`},silent:!1,tag:"peta-bicara-basic"});return r.addEventListener("click",()=>{window.focus(),r.close(),window.router?window.router.navigateTo("/"):window.location.href=`${this.basePath}/`}),r.addEventListener("error",a=>{console.error("Notification error:",a)}),setTimeout(()=>{r.close()},8e3),console.log("Basic notification shown successfully"),!0}catch(o){return console.error("Error showing basic notification:",o),!1}}async isSubscribedToStoryAPI(){try{return localStorage.getItem("token")?!!localStorage.getItem("story-api-subscription-id"):!1}catch(e){return console.error("Error checking Story API subscription:",e),!1}}async saveSubscriptionLocally(e){try{window.indexedDBHelper?(await window.indexedDBHelper.saveSetting("pushSubscription",{endpoint:e.endpoint,keys:{p256dh:this.arrayBufferToBase64(e.getKey("p256dh")),auth:this.arrayBufferToBase64(e.getKey("auth"))},subscribedAt:new Date().toISOString(),basePath:this.basePath,storyApiSubscriptionId:localStorage.getItem("story-api-subscription-id")}),console.log("Push subscription saved locally")):(console.warn("IndexedDB helper not available, using localStorage fallback"),localStorage.setItem("pwa-push-subscription",JSON.stringify({endpoint:e.endpoint,subscribedAt:new Date().toISOString(),basePath:this.basePath})))}catch(t){console.error("Error saving subscription locally:",t)}}async removeSubscriptionLocally(){try{window.indexedDBHelper?(await window.indexedDBHelper.saveSetting("pushSubscription",null),console.log("Push subscription removed locally")):localStorage.removeItem("pwa-push-subscription"),localStorage.removeItem("story-api-subscription-id")}catch(e){console.error("Error removing subscription locally:",e)}}async getSubscriptionInfo(){try{const e=await this.isSubscribed(),t=await this.isSubscribedToStoryAPI(),i=window.indexedDBHelper?await window.indexedDBHelper.getSetting("pushSubscription"):JSON.parse(localStorage.getItem("pwa-push-subscription")||"null");return{isSubscribed:e,isSubscribedToStoryAPI:t,hasPermission:Notification.permission==="granted",isSupported:this.isSupported,subscription:this.subscription,localSubscription:i,basePath:this.basePath,storyApiSubscriptionId:localStorage.getItem("story-api-subscription-id")}}catch(e){return console.error("Error getting subscription info:",e),{isSubscribed:!1,isSubscribedToStoryAPI:!1,hasPermission:!1,isSupported:this.isSupported,subscription:null,localSubscription:null,basePath:this.basePath}}}async isSubscribed(){try{return this.registration||await this.init(),this.subscription=await this.registration.pushManager.getSubscription(),!!this.subscription}catch(e){return console.error("Error checking subscription status:",e),!1}}urlBase64ToUint8Array(e){const t="=".repeat((4-e.length%4)%4),i=(e+t).replace(/-/g,"+").replace(/_/g,"/"),o=window.atob(i),r=new Uint8Array(o.length);for(let a=0;a<o.length;++a)r[a]=o.charCodeAt(a);return r}arrayBufferToBase64(e){const t=new Uint8Array(e);let i="";return t.forEach(o=>i+=String.fromCharCode(o)),window.btoa(i)}async testNotification(){try{if(console.log("Testing notification..."),Notification.permission!=="granted")throw new Error("Izin notifikasi belum diberikan");return this.registration&&(console.log("Showing test notification via Service Worker..."),await this.showServiceWorkerNotification("🧪 Test Notifikasi","Ini adalah test notifikasi dari Peta Bicara! Notifikasi melalui Service Worker berfungsi dengan baik!",{tag:"test-notification",data:{test:!0,url:`${this.basePath}/`}}))?!0:(console.log("Showing test notification via basic notification..."),this.showBasicNotification("🧪 Test Notifikasi","Ini adalah test notifikasi dari Peta Bicara! Notifikasi dasar berfungsi dengan baik!"))}catch(e){return console.error("Error testing push notification:",e),!1}}}const u=new B;document.addEventListener("DOMContentLoaded",async()=>{console.log("Initializing push notifications with Story API...");try{await u.init(),(window.indexedDBHelper?await window.indexedDBHelper.getSetting("pushSubscription"):JSON.parse(localStorage.getItem("pwa-push-subscription")||"null"))&&Notification.permission==="granted"&&(await u.isSubscribed()||(console.log("Re-subscribing to push notifications..."),await u.subscribe()))}catch(n){console.error("Failed to initialize push notifications:",n)}});window.pushNotificationHelper=u;class N{constructor(){this.swPath="/Submission-Intermediate/sw.js",this.registration=null,this.deferredPrompt=null,this.isStandalone=window.matchMedia("(display-mode: standalone)").matches}async init(){if(!this.isServiceWorkerSupported()){console.log("Service Worker not supported");return}try{await this.register(),this.setupInstallPrompt(),this.setupUpdateCheck(),this.handleAppInstalled(),console.log("Service Worker registration complete")}catch(e){console.error("Service Worker registration failed:",e)}}isServiceWorkerSupported(){return"serviceWorker"in navigator}async register(){try{return this.registration=await navigator.serviceWorker.register(this.swPath,{scope:"/Submission-Intermediate/"}),console.log("Service Worker registered:",this.registration),this.registration.addEventListener("updatefound",()=>{console.log("Service Worker update found");const e=this.registration.installing;e.addEventListener("statechange",()=>{e.state==="installed"&&navigator.serviceWorker.controller&&this.showUpdateNotification()})}),navigator.serviceWorker.addEventListener("controllerchange",()=>{console.log("Service Worker controller changed"),window.location.reload()}),this.registration}catch(e){throw console.error("Service Worker registration error:",e),e}}setupInstallPrompt(){window.addEventListener("beforeinstallprompt",e=>{console.log("beforeinstallprompt event fired"),e.preventDefault(),this.deferredPrompt=e,this.showInstallPromotion(),console.log("Install prompt platforms:",e.platforms)})}showInstallPromotion(){const e=localStorage.getItem("pwa-install-dismissed"),t=e?new Date(e):null;if((t?(new Date-t)/(1e3*60*60*24):1/0)<7){console.log("Install prompt recently dismissed");return}const o=document.createElement("div");o.id="installBanner",o.className="install-banner",o.innerHTML=`
    ...
    `,o.textContent=e,!document.getElementById("toastAnimation")){const r=document.createElement("style");r.id="toastAnimation",r.textContent=`
        @keyframes slideIn {
    ...
    `,o.innerHTML=i,document.body.appendChild(o),o.addEventListener("click",r=>{r.target===o&&o.remove()})}getBrowserName(){const e=navigator.userAgent;return e.includes("Chrome")?"Chrome":e.includes("Firefox")?"Firefox":e.includes("Safari")?"Safari":e.includes("Edge")?"Edge":"Unknown"}showNotificationMessage(e,t="info"){console.log(`NotificationUIHelper: Message - ${t}: ${e}`);const i=document.querySelector(".notification-message");i&&i.remove();const o=document.createElement("div");o.className="notification-message",o.style.cssText=`
      ...
      max-width: 300px;
    `;const r={success:"#28a745",error:"#dc3545",warning:"#ffc107",info:"#17a2b8"};o.style.background=r[t]||r.info,o.textContent=e,document.body.appendChild(o),setTimeout(()=>{o.parentNode&&(o.style.animation="slideOutRight 0.3s ease",setTimeout(()=>o.remove(),300))},4e3)}}const k=new F;document.addEventListener("DOMContentLoaded",()=>{setTimeout(async()=>{console.log("NotificationUIHelper: Starting initialization"),await k.init(),console.log("NotificationUIHelper: Ready for use")},2e3)});if(!document.getElementById("slideAnimations")){const n=document.createElement("style");n.id="slideAnimations",n.textContent=`
  ...
  `,document.head.appendChild(n)}window.notificationUIHelper=k;class M{constructor(){this.baseUrl="https://story-api.dicoding.dev/v1",this.isOnline=navigator.onLine,this.requestQueue=[],window.addEventListener("online",()=>{console.log("Network back online"),this.isOnline=!0,this.processRequestQueue()}),window.addEventListener("offline",()=>{console.log("Network went offline"),this.isOnline=!1})}async fetchWithTimeout(e,t={},i=8e3){if(!this.isOnline&&!navigator.onLine)throw new Error("No internet connection");const o=new AbortController,r=setTimeout(()=>{o.abort()},i);try{const a=await fetch(e,{...t,signal:o.signal});return clearTimeout(r),a}catch(a){throw clearTimeout(r),a.name==="AbortError"?new Error("Request timeout"):a}}async fetchWithRetry(e,t={},i=2){let o;for(let r=0;r<=i;r++)try{return console.log(`API request attempt ${r+1}/${i+1}: ${e}`),await this.fetchWithTimeout(e,t)}catch(a){if(o=a,console.warn(`API request attempt ${r+1} failed:`,a.message),a.message.includes("No internet connection")||a.message.includes("401")||a.message.includes("403"))break;if(r<i){const s=Math.min(1e3*Math.pow(2,r),5e3);console.log(`Waiting ${s}ms before retry...`),await new Promise(l=>setTimeout(l,s))}}throw o}async register(e,t,i){try{if(console.log("Registering user:",e,t),!this.isOnline)throw new Error("Tidak dapat mendaftar saat offline. Silakan coba lagi saat online.");const r=await(await this.fetchWithRetry(`${this.baseUrl}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e,email:t,password:i})})).json();if(console.log("Register response:",r),r.error)throw new Error(r.message);return r}catch(o){throw console.error("Registration error:",o),o.message.includes("No internet connection")||o.message.includes("timeout")?new Error("Tidak ada koneksi internet. Periksa koneksi Anda dan coba lagi."):new Error(`Gagal mendaftar: ${o.message}`)}}async login(e,t){try{if(console.log("Login attempt for:",e),!this.isOnline)throw new Error("Tidak dapat login saat offline. Silakan coba lagi saat online.");const o=await(await this.fetchWithRetry(`${this.baseUrl}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t})})).json();if(console.log("Login response:",o),o.error)throw new Error(o.message);return localStorage.setItem("token",o.loginResult.token),localStorage.setItem("user",JSON.stringify(o.loginResult)),o}catch(i){throw console.error("Login error:",i),i.message.includes("No internet connection")||i.message.includes("timeout")?new Error("Tidak ada koneksi internet. Periksa koneksi Anda dan coba lagi."):new Error(`Gagal login: ${i.message}`)}}async getAllStories(){try{const e=localStorage.getItem("token");if(console.log("Getting stories with token:",e?"Token exists":"No token"),!e)throw new Error("Token tidak ditemukan. Silakan login terlebih dahulu.");if(!this.isOnline){console.log("Offline - trying to get cached stories");const o=await this.getCachedStories();if(o&&o.length>0)return console.log("Returning cached stories:",o.length),o;throw new Error("Tidak ada koneksi internet dan tidak ada data tersimpan")}const i=await(await this.fetchWithRetry(`${this.baseUrl}/stories?location=1`,{headers:{Authorization:`Bearer ${e}`}})).json();if(console.log("Stories response:",i),i.error)throw new Error(i.message);return await this.cacheStories(i.listStory),i.listStory}catch(e){if(console.error("Error fetching stories:",e),e.message.includes("Token tidak ditemukan"))return window.router&&window.router.navigateTo("/masuk"),[];if(this.isOnline){console.log("Online request failed, trying cached stories");const t=await this.getCachedStories();if(t&&t.length>0)return console.log("Returning cached stories as fallback"),t}return[]}}async addNewStory(e,t,i,o){try{const r=localStorage.getItem("token");if(console.log("Adding new story with token:",r?"Token exists":"No token"),!r)throw new Error("Token tidak ditemukan. Silakan login terlebih dahulu.");const a=new FormData;if(a.append("description",e),a.append("photo",t,"photo.jpg"),i!==null&&o!==null&&(a.append("lat",i),a.append("lon",o)),!this.isOnline)return console.log("Offline - queueing story for later upload"),await this.queueOfflineStory({description:e,photoBlob:t,lat:i,lon:o,token:r}),{error:!1,message:"Cerita disimpan offline dan akan diunggah saat online"};console.log("Sending request to add story");const l=await(await this.fetchWithRetry(`${this.baseUrl}/stories`,{method:"POST",headers:{Authorization:`Bearer ${r}`},body:a})).json();if(console.log("Add story response:",l),l.error)throw new Error(l.message);return l}catch(r){if(console.error("Error adding story:",r),r.message.includes("No internet connection")||r.message.includes("timeout")){console.log("Network error - queueing story for later upload");const a=localStorage.getItem("token");return await this.queueOfflineStory({description:e,photoBlob:t,lat:i,lon:o,token:a}),{error:!1,message:"Cerita disimpan offline dan akan diunggah saat online"}}throw new Error(`Gagal menambah cerita: ${r.message}`)}}async cacheStories(e){try{if(window.indexedDBHelper)await window.indexedDBHelper.saveOfflineStories(e),console.log("Stories cached in IndexedDB");else{const t=JSON.stringify(e);t.length<2e6&&(localStorage.setItem("cached_stories",t),localStorage.setItem("cached_stories_timestamp",Date.now().toString()),console.log("Stories cached in localStorage"))}}catch(t){console.warn("Failed to cache stories:",t)}}async getCachedStories(){try{if(window.indexedDBHelper)return await window.indexedDBHelper.getOfflineStories()||[];{const e=localStorage.getItem("cached_stories"),t=localStorage.getItem("cached_stories_timestamp");return e&&t&&Date.now()-parseInt(t)<24*60*60*1e3?JSON.parse(e):[]}}catch(e){return console.warn("Failed to get cached stories:",e),[]}}async queueOfflineStory(e){try{const t={id:Date.now().toString(),...e,timestamp:Date.now()};if(window.indexedDBHelper)await window.indexedDBHelper.saveOfflineStory(t),console.log("Story queued in IndexedDB");else{const i=JSON.parse(localStorage.getItem("offline_story_queue")||"[]");i.push(t),localStorage.setItem("offline_story_queue",JSON.stringify(i)),console.log("Story queued in localStorage")}if("serviceWorker"in navigator&&"sync"in window.ServiceWorkerRegistration.prototype)try{await(await navigator.serviceWorker.ready).sync.register("background-sync-story"),console.log("Background sync registered")}catch(i){console.warn("Background sync registration failed:",i)}}catch(t){throw console.error("Failed to queue offline story:",t),t}}async processRequestQueue(){if(this.isOnline)try{console.log("Processing offline story queue...");let e=[];if(window.indexedDBHelper?e=await window.indexedDBHelper.getOfflineStoryQueue()||[]:e=JSON.parse(localStorage.getItem("offline_story_queue")||"[]"),e.length===0){console.log("No offline stories to process");return}console.log(`Processing ${e.length} offline stories`);for(const t of e)try{const i=new FormData;if(i.append("description",t.description),i.append("photo",t.photoBlob,"photo.jpg"),t.lat!==null&&t.lon!==null&&(i.append("lat",t.lat),i.append("lon",t.lon)),(await this.fetchWithTimeout(`${this.baseUrl}/stories`,{method:"POST",headers:{Authorization:`Bearer ${t.token}`},body:i})).ok){if(console.log("Offline story uploaded successfully:",t.id),window.indexedDBHelper)await window.indexedDBHelper.removeOfflineStory(t.id);else{const r=e.filter(a=>a.id!==t.id);localStorage.setItem("offline_story_queue",JSON.stringify(r))}"serviceWorker"in navigator&&navigator.serviceWorker.ready.then(r=>{r.showNotification("Cerita Diunggah",{body:"Cerita offline berhasil diunggah!",icon:"/Submission-Intermediate/icons/icon-192x192.png"})})}}catch(i){console.error("Failed to upload offline story:",t.id,i)}}catch(e){console.error("Error processing offline queue:",e)}}isAuthenticated(){return!!localStorage.getItem("token")}logout(){localStorage.removeItem("token"),localStorage.removeItem("user")}getCurrentUser(){const e=localStorage.getItem("user");return e?JSON.parse(e):null}getNetworkStatus(){return{isOnline:this.isOnline&&navigator.onLine,lastCheck:Date.now()}}}const _=new M;window.apiService=_;class D{constructor(e){this.apiService=e,console.log("StoryModel initialized with apiService")}async getAllStories(){try{console.log("StoryModel.getAllStories: Requesting stories from API");const e=await this.apiService.getAllStories();return!e||!Array.isArray(e)?(console.warn("StoryModel: Invalid stories data received from API"),[]):(console.log(`StoryModel: Successfully retrieved ${e.length} stories`),e)}catch(e){throw console.error("StoryModel: Error getting stories:",e),new Error(`Failed to load stories: ${e.message}`)}}async addNewStory(e,t,i,o){try{if(console.log("StoryModel.addNewStory: Adding new story"),!e||!t)throw new Error("Description and photo are required");return await this.apiService.addNewStory(e,t,i,o)}catch(r){throw console.error("StoryModel: Error adding story:",r),new Error(`Failed to add story: ${r.message}`)}}}window.StoryModel=D;class x{constructor(){this.render()}render(){const e=document.querySelector("footer");if(e&&(e.innerHTML=`
      <div class="container">
    ...
    `,!document.getElementById("compactFooterStyles"))){const t=document.createElement("style");t.id="compactFooterStyles",t.textContent=`
      ...
      `,document.head.appendChild(t)}}}new x;window.Footer=x;class S{constructor(){this._bindEvents()}_bindEvents(){document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("navToggle"),t=document.querySelector(".nav-menu");document.getElementById("authNavItem"),e&&t&&e.addEventListener("click",()=>{t.classList.toggle("active");const i=e.getAttribute("aria-expanded")==="true";e.setAttribute("aria-expanded",!i),e.innerHTML=i?'<i class="fas fa-bars" aria-hidden="true"></i>':'<i class="fas fa-times" aria-hidden="true"></i>'}),document.addEventListener("click",i=>{t&&t.classList.contains("active")&&!i.target.closest(".nav-menu")&&!i.target.closest("#navToggle")&&(t.classList.remove("active"),e&&(e.setAttribute("aria-expanded","false"),e.innerHTML='<i class="fas fa-bars" aria-hidden="true"></i>'))}),this.updateAuthNavItem(),window.addEventListener("hashchange",()=>{this.updateActiveLink()}),this.updateActiveLink()})}updateAuthNavItem(){const e=document.getElementById("authNavItem");if(e)if(localStorage.getItem("token")!==null){e.innerHTML=`
        ...
        `}updateActiveLink(){const e=window.location.hash||"#/";document.querySelectorAll(".nav-menu a").forEach(i=>{i.getAttribute("href")===e?(i.classList.add("active"),i.setAttribute("aria-current","page")):(i.classList.remove("active"),i.removeAttribute("aria-current"))})}}new S;window.Navbar=S;class I extends HTMLElement{constructor(){super()}set story(e){this._story=e,this.render()}formatDate(e){const t={year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"};return new Date(e).toLocaleDateString("id-ID",t)}render(){const{name:e,photoUrl:t,description:i,createdAt:o,lat:r,lon:a}=this._story;this.innerHTML=`
      <article class="story-card fadeIn">
        ...
        <div class="story-content">
          ...
          <div class="story-info">
            ...
            ${r&&a?`
              <div class="story-location" data-lat="${r}" data-lon="${a}">
    ...
    `;const s=this.querySelector(".story-location");s&&s.addEventListener("click",l=>{l.preventDefault();const p=parseFloat(s.dataset.lat),d=parseFloat(s.dataset.lon),g=new CustomEvent("story-location-click",{detail:{lat:p,lon:d,name:e,description:i},bubbles:!0});this.dispatchEvent(g)})}}customElements.define("story-card",I);window.StoryCard=I;class z{constructor(){this._container=document.querySelector("#mainContent"),this._map=null,this._marker=null,this._position={lat:null,lon:null},this._cameraActive=!1,this._canvas=document.createElement("canvas"),this._presenter=null,this._setupNavigationListener()}setPresenter(e){this._presenter=e}_setupNavigationListener(){window.addEventListener("hashchange",()=>{this._cameraActive&&(console.log("Navigation detected, stopping camera"),this._stopCameraAndCleanup())}),window.addEventListener("beforeunload",()=>{this._cameraActive&&(console.log("Page unload detected, stopping camera"),this._stopCameraAndCleanup())})}_stopCameraAndCleanup(){console.log("Stopping camera and cleaning up resources"),cameraHelper.stopCamera(),this._cameraActive=!1;const e=document.getElementById("cameraFeed");e&&(e.srcObject=null)}render(){console.log("Rendering add story page"),this._container.innerHTML=`
      <div class="container">
    ...
    `,this._addStyles(),this.storiesContainer=document.getElementById("storiesContainer"),this.loadingIndicator=document.getElementById("loadingIndicator"),this.storiesContainer||console.error("storiesContainer element not found"),this.loadingIndicator||console.error("loadingIndicator element not found"),this.showLoading(),setTimeout(()=>{this.presenter?(console.log("HomeView: Requesting stories from presenter"),this.presenter.loadStories()):(console.error("Presenter not set in HomeView"),this.renderError("Error: Unable to load stories. Presenter not initialized."))},100)}renderStories(e){if(console.log("HomeView.renderStories called with",e?e.length:0,"stories"),this.hideLoading(),!e||e.length===0){this.renderEmptyState();return}this.storiesContainer?(this.storiesContainer.innerHTML="",e.forEach(t=>{const i=document.createElement("div");i.className="story-card",i.innerHTML=`
      ...
      `,document.head.appendChild(e)}}}window.HomeView=R;console.log("HomeView exported to window");class V{constructor(){this.container=document.querySelector("#mainContent"),this.presenter=null}setPresenter(e){this.presenter=e}render(){this.container.innerHTML=`
        ...
        `,document.head.appendChild(e)}_initListeners(){const e=document.querySelector("#registerForm");if(e){console.log("Found register form, attaching event listener");const t=e.cloneNode(!0);e.parentNode.replaceChild(t,e),t.addEventListener("submit",async i=>{if(i.preventDefault(),console.log("Register form submitted"),this.presenter){const o=document.querySelector("#name").value,r=document.querySelector("#email").value,a=document.querySelector("#password").value;this.presenter.onRegisterSubmit(o,r,a)}else console.error("Register presenter not set"),this.showAlert("Terjadi kesalahan sistem")})}else console.error("Register form tidak ditemukan");setTimeout(()=>{const t=document.querySelector('a[href="#/masuk"]');t&&t.addEventListener("click",i=>{i.preventDefault(),this.presenter&&this.presenter.navigateToLogin()})},100)}validateForm(e,t,i){return!e||!t||!i?(this.showAlert("Semua field wajib diisi!"),!1):i.length<6?(this.showAlert("Password minimal 6 karakter!"),!1):!0}showLoading(e){const t=document.getElementById("registerButton");e?(t.classList.add("btn-loading"),t.disabled=!0):(t.classList.remove("btn-loading"),t.disabled=!1)}showAlert(e){const t=document.getElementById("alertContainer");if(!t)return;const i=document.querySelector(".alert");i&&i.remove();const o=document.createElement("div");o.className="alert alert-danger",o.style.cssText="position: fixed; top: 80px; left: 50%; transform: translateX(-50%); background-color: #f44336; color: white; padding: 12px 24px; border-radius: 4px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); z-index: 1050; max-width: 90%;",o.innerHTML=`<i class="fas fa-exclamation-circle"></i> ${e}`,t.appendChild(o),setTimeout(()=>{o.style.opacity="0",o.style.transition="opacity 0.5s ease",setTimeout(()=>o.remove(),500)},3e3)}showSuccess(e){this.showLoading(!1);const t=document.getElementById("alertContainer");if(!t)return;const i=document.querySelector(".alert");i&&i.remove();const o=document.createElement("div");o.className="alert alert-success",o.style.cssText="position: fixed; top: 80px; left: 50%; transform: translateX(-50%); background-color: #4CAF50; color: white; padding: 12px 24px; border-radius: 4px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); z-index: 1050; max-width: 90%;",o.innerHTML=`<i class="fas fa-check-circle"></i> ${e}`,t.appendChild(o),setTimeout(()=>{o.style.opacity="0",o.style.transition="opacity 0.5s ease",setTimeout(()=>o.remove(),500)},2e3)}scheduleNavigation(e,t=1e3){setTimeout(e,t)}}window.RegisterView=$;console.log("RegisterView exported to window");class q{constructor(){this._container=document.querySelector("#mainContent"),this._map=null,this._markers=[],this._presenter=null}setPresenter(e){this._presenter=e}render(){console.log("Rendering map page"),this._container.innerHTML="",this._container.innerHTML=`
    ...
    `,document.head.appendChild(e)}_setupEvents(){const e=document.getElementById("exportFavoritesBtn"),t=document.getElementById("clearDataBtn");e&&e.addEventListener("click",()=>{this.presenter&&this.presenter.exportFavorites()}),t&&t.addEventListener("click",()=>{this.presenter&&this.presenter.clearAllData()})}_initializeFeatures(){this._initNotificationSettings(),this._loadDataStats(),this._loadAppStatus()}_initNotificationSettings(){const e=document.getElementById("notificationSettings");e&&window.notificationUIHelper?window.notificationUIHelper.createNotificationSettingsUI(e):e.innerHTML=`
      ...
      `}async _loadDataStats(){var t,i,o;const e=document.getElementById("dataStats");try{if(window.indexedDBHelper){const r=await window.indexedDBHelper.getDatabaseInfo();e.innerHTML=`
          <div class="stats-grid">
            <div class="stat-item">
              <span class="stat-label">Favorit:</span>
              <span class="stat-value">${((t=r.stores.favorites)==null?void 0:t.count)||0}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Offline Stories:</span>
              <span class="stat-value">${((i=r.stores.offlineStories)==null?void 0:i.count)||0}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Settings:</span>
              <span class="stat-value">${((o=r.stores.settings)==null?void 0:o.count)||0}</span>
            </div>
          </div>
        `}else throw new Error("IndexedDB helper not available")}catch(r){console.error("Error loading data stats:",r),e.innerHTML=`
        <div class="error-message">
          <i class="fas fa-exclamation-circle"></i>
    ...
    `,document.getElementById("retryButton").addEventListener("click",()=>{this.presenter?this.presenter.reloadPage():window.location.reload()})}setupNavigation(){const e=document.getElementById("navToggle"),t=document.querySelector(".nav-menu");e&&t&&e.addEventListener("click",()=>{const i=e.getAttribute("aria-expanded")==="true";e.setAttribute("aria-expanded",!i),t.classList.toggle("active")}),document.querySelectorAll(".nav-menu a").forEach(i=>{i.addEventListener("click",()=>{t&&t.classList.contains("active")&&(t.classList.remove("active"),e&&e.setAttribute("aria-expanded","false"))})})}updateAuthNavItem(e){const t=document.getElementById("authNavItem");t&&(e?(t.innerHTML='<a href="#" id="logoutBtn"><i class="fas fa-sign-out-alt" aria-hidden="true"></i> Keluar</a>',document.getElementById("logoutBtn").addEventListener("click",i=>{i.preventDefault(),this.presenter&&this.presenter.logout()})):t.innerHTML='<a href="#/masuk"><i class="fas fa-sign-in-alt" aria-hidden="true"></i> Masuk</a>'),this._updateSettingsNavigation(e)}_updateSettingsNavigation(e){const t=document.querySelector(".nav-menu");if(!t)return;let i=t.querySelector('a[href="#/pengaturan"]');if(e&&!i){const o=document.getElementById("authNavItem");if(o){const r=document.createElement("li");r.className="auth-required-nav",r.innerHTML=`
        ...
        `),n()):(console.log(`⏳ Waiting for dependencies... (${e}/${t})`),console.log("🔍 Missing classes:",r),setTimeout(i,100))};i()})}function E(n){const e=document.getElementById("mainContent");e&&(e.innerHTML=`
