...
export default class AddStoryPage {
    ...
    return `
      <section class="container" role="main" aria-labelledby="add-title">
        ...
        <form id="add-story-form" class="form-container">
          ...
          <button type="button" id="camera-btn">Ambil dari Kamera</button>
  ...
  async handleSubmit() {
    ...
    if (!description || description.length < 5 || !photoInput.files[0]) {
      document.getElementById('form-message').textContent = 'Lengkapi form dengan valid.';
      return;
    }
    ...
    try {
      await addStory(formData, false); // Auth mode
      document.getElementById('form-message').textContent = 'Cerita berhasil ditambahkan!';
      this.form.reset();
      this.stopCamera();
